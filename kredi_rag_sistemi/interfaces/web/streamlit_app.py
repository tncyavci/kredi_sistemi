...
# Fix multiprocessing issues BEFORE any other imports
...
from src.core.rag_engine import KrediRAG, get_rag_instance
from src.core.document_processor import PDFProcessor
            ...
            if st.button("Seçilen Dosyaları İşle", type="primary"):
                ...
                with st.status("Dosyalar işleniyor...", expanded=True) as status:
                    try:
                        ...
                        # PDF'leri işle
                        from src.utils.preprocessing.enhanced_pdf_processor import EnhancedPdfProcessor
                            ...
                            with st.expander(f"Referans belgeleri ({len(result['source_documents'])})"):
                                for i, doc in enumerate(result["source_documents"]):
                                    ...
                                    with col1:
                                        ...
                                        # Fix progress bar error - ensure confidence is between 0-1
                                        distance = doc.get("distance", 1.0)
                                        if distance < 0:
                                            confidence = 1.0  # Perfect match
                                        elif distance > 2.0:
                                            confidence = 0.0  # Poor match
                                        else:
                                        ...
                                        st.caption(f"Benzerlik: {confidence:.2f}")
